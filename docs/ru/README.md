...
- **В слое памяти была реальная течь.** WASM-ФС sql.js (MEMFS) копит по полному образу БД на каждое открытие — прод-инстанс дорос до ~36 ГБ RSS за 6 недель. Сдерживаем **RSS-watchdog'ом** (грейсфул-респавн child'а по порогу RAM, `RUFLO_CHILD_MAX_RSS_MB`, дефолт 3000) и патчим корень на этапе сборки. Полный разбор с heap-снимком: [`ruvnet/ruflo#2432`](https://github.com/ruvnet/ruflo/issues/2432).
- **История безопасности (важно):** в ruflo [issue #1375](https://github.com/ruvnet/ruflo/issues/1375) сообщали о вредоносном обфусцированном preinstall-скрипте и скрытой prompt-инъекции в описаниях MCP-инструментов — **в старых версиях (3.1.0-alpha.55 – 3.5.2)**. Поскольку хаб ставит пакет и раздаёт описания клиентам, мы проверили версию, которую шипим (3.12.4): preinstall-скриптов нет, все 302 описания чистые. В текущих версиях считаем вылеченным — но **пиньте версию и проверяйте сами.**
...
| GET | `/health` | Статус сервера (`{"status":"ok","tools":302, ...}`) — включает статистику RSS-watchdog'а (`childRssMB`, `childRespawnCount`) |
