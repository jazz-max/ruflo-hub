...
> ⚠️ **Сначала прочти это.** «Swarm / hive-mind / оркестрация агентов» в ruflo — это **не реальное мультиагентное выполнение**, а координационные *записи*, никакие процессы не спавнятся (этот документ ровно это и объясняет, независимые аудиты подтверждают). `ruflo-hub` ни на что из этого **не опирается**; реальная ценность проекта — слой памяти. Эта страница оставлена как точное описание поведения swarm-учёта, **а не** как одобрение его в качестве рабочей фичи. См. README → [Что это на самом деле](README.md#что-это-на-самом-деле).
