...
### Docs
- **Заметки про безопасность и течь обновлены под фактически шипаемую версию (3.14.2).** v1.3.0 шипит ruflo 3.14.2 (апстрим-фикс течи #2432), но честные заметки в README всё ещё ссылались на прежние `3.12.4` / `302 tools`. Перепроверили 3.14.2 и обновили README (en + ru): заметка про течь теперь отдаёт должное апстрим-фиксу корня (`closePriorIfAny` в `@claude-flow/memory@3.0.0-alpha.21`), RSS-watchdog оставлен как страховка; security-заметка переаудирована по шипаемому дереву — `preinstall`-хуков нет нигде, `install`-скрипты — только стандартные нативные сборки (better-sqlite3/argon2/bcrypt), `postinstall`-скрипты (agentdb, `@claude-flow/cli`, protobufjs) безобидны (прочитаны целиком — без обфускации/сети/eval), и все **305** живых описаний инструментов чисты (0 скрытого/bidi-Unicode; детерминированный + 8-агентный адверсариальный семантический свип нашли 0 инъекций); примеры `/health` исправлены на `tools: 305`.

## [1.3.0] — 2026-06-26

### Fixed
- **Утечка памяти (#2432) исправлена апстримом в ruflo 3.14.2.** ruvnet подтвердил настоящий корень: `@claude-flow/memory` `ControllerRegistry.initController()` заменял запись в Map, не закрывая предыдущий контроллер (каждый ре-init оставлял осиротевший `SqlJsRvfBackend`/`SQL.Database` → `dbfile_*` в MEMFS sql.js). Исправлено в `@claude-flow/memory@3.0.0-alpha.21` (`closePriorController()` перед `controllers.set()`). Сборка на `ruflo@latest` это подтягивает; RSS-watchdog (1.2.0) теперь безвредная страховка, а не основная защита. Примечание: патч `createDatabase`-кэша из 1.2.1 бил не в тот слой (ruvnet подтвердил, что `createDatabase` был в порядке) — оставлен как безвредный best-effort no-op.

### Added
- **Запекание ONNX-модели эмбеддингов в образ.** ruflo качает `all-MiniLM-L6-v2` (~90 МБ) с HuggingFace при первом запуске, кэшируя в `/root/.ruvector/models` — это НЕ volume, поэтому каждое пересоздание контейнера теряло модель и качало заново. На флапающей сети скачивание не успевало за 60-секундный MCP-таймаут → `server.mjs` уходил в крэш-луп (рестарт убивал недокачку). Dockerfile теперь прогревает модель на этапе сборки — старт мгновенный и устойчив к пересозданию.

### Docs
- **Честное перепозиционирование README + docs.** Независимый аудит ([r/ClaudeAI](https://www.reddit.com/r/ClaudeAI/comments/1sckiy8/)) показал, что большинство из 300+ MCP-инструментов ruflo — нерабочие заглушки; реально работает слой памяти. README (en + ru) теперь ведёт от этого: новая секция «Что это на самом деле» — что реально (память: эмбеддинги + HNSW + SQLite + auto-memory) vs театр (swarm/neural/agent-заглушки), и что `ruflo-hub` намеренно использует только память. Добавлены честные заметки про течь (#2432 + RSS-watchdog), security-историю #1375 (ограничена версиями ≤3.5.2; в шипаемой 3.12.4 проверено — чисто) и WAL-безопасный бэкап. Убран «250+ tools» как продающий тезис; счётчик в `/health` исправлен на 302. В `docs/swarm-management.md` (en + ru) добавлен баннер: swarm/hive-mind — координационные записи, не реальное выполнение, и проект на них не опирается.
